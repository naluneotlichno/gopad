...
func NextDate(now time.Time, dateStr string, repeat string) (string, error) {
    ...
    if err != nil {
        // Если парсинг не удался, тесты ждут "пустой ответ" (ошибку)
    ...
    if repeat == "y" {
        ...
        // Если parsedDate <= now, крутим year++, пока не уйдём за now
        for !nextDate.After(now) {
            nextDate = addYearFixLeap(parsedDate, nextDate)
        }
        // Если изначально parsedDate > now, значит нужно сделать ОДНУ итерацию
        // (чтобы получить "следующую" дату) — НО только если nextDate == parsedDate.
        // На самом деле это покроется тем же циклом, но давай различим:
        if parsedDate.After(now) {
            // nextDate до цикла = parsedDate, он не зашёл в for, значит сделаем одну итерацию вручную
            nextDate = addYearFixLeap(parsedDate, nextDate)
        }
    ...
    if strings.HasPrefix(repeat, "d ") {
        ...
        if err != nil {
            return "", fmt.Errorf("некорректное число дней")
        }
        ...
        // Если parsedDate > now, значит делаем одну итерацию (следующая дата)
    ...
    // 3) Если какие-то другие форматы (w ..., m ...), это уже отдельная задача
    // у вас в коде может быть своя логика
...
func addYearFixLeap(originalDate, currentDate time.Time) time.Time {
    ...
    // Если исходная дата была 29.02,
    // и Go скастил нам 28.02, то двигаем на 01.03
